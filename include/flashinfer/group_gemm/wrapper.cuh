...
cudaError_t CutlassSegmentGEMMWrapper(CutlassSegmentGEMMHandler* handler, DTypeIn* x, DTypeIn* w,
  ...
  if (compute_capacity.first == 8) {
    ...
    using TileShape =
        typename std::conditional<is_fp8, Shape<_256, _128, _128>,
                                  Shape<_128, _128, _64>>;  // Threadblock-level tile size
    using ClusterShape =
        typename std::conditional<is_fp8, Shape<_2, _2, _1>,
                                  Shape<_2, _1, _1>>;  // Shape of the threadblocks in a cluster
    ...
    AlignedAllocator allocator(handler->GetIntWorkspace(), handler->GetWorkspaceSizeInBytes());
    cutlass::gemm::GemmCoord* problem_sizes_device =
        allocator.aligned_alloc<cutlass::gemm::GemmCoord>(
            batch_size * sizeof(cutlass::gemm::GemmCoord), 16, "problem_sizes_device");
    DTypeIn** x_data =
    ...
    arguments = typename Gemm::Arguments{cutlass::gemm::GemmUniversalMode::kGrouped,
                                         {batch_size, problem_sizes_device, nullptr},
    ...
    auto workspace = float_allocator.aligned_alloc<uint8_t>(workspace_size, 64,
