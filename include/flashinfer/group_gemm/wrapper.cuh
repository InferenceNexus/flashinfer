  ...
  if (compute_capacity.first == 8) {
    if constexpr (sizeof(DTypeIn) != 2) {
      ...
      return cudaErrorNotSupported;
        ...
        // SM80 grouped gemm
        AlignedAllocator allocator(handler->GetIntWorkspace(),
        handler->GetIntWorkspaceSizeInBytes()); cutlass::gemm::GemmCoord* problem_sizes_device =
    ...
    using TileShape =
        typename std::conditional<is_fp8, Shape<_256, _128, _64>,
                                  Shape<_128, _64, _64>>::type;  // Threadblock-level tile size
    using ClusterShape = typename std::conditional<is_fp8, Shape<_2, _2, _1>, Shape<_2, _1, _1>>::
    ...
    ProblemShape::UnderlyingProblemShape* problem_sizes_device =
        allocator.aligned_alloc<ProblemShape::UnderlyingProblemShape>(
            batch_size * sizeof(ProblemShape::UnderlyingProblemShape), 16, "problem_sizes_device");
    DTypeIn** x_data =
        allocator.aligned_alloc<DTypeIn*>(batch_size * sizeof(DTypeIn*), 16, "x_data");
    DTypeIn** w_data =
        allocator.aligned_alloc<DTypeIn*>(batch_size * sizeof(DTypeIn*), 16, "w_data");
    ...
    AlignedAllocator float_allocator(handler->GetFloatWorkspace(),
                                     handler->GetFloatWorkspaceSizeInBytes());
    auto workspace_ptr =
        float_allocator.aligned_alloc<void>(workspace_size, 64, "sm90_group_gemm_float_workspace");
